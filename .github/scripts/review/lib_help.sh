...
ℹ️ 모든 AI 리뷰는 기본적으로 **작성자/요청자 개인 키**로 동작하고, 한도(429) 도달 시에만 **공통 키로 자동 폴백**해요(미등록 사용자는 처음부터 공통 키). PR을 열면 자동 리뷰 1회, 이후 **푸시마다 자동 코드 제안(/improve) + 해결된 코멘트 자동 정리**가 실행됩니다. 푸시 자동 제안은 `/auto-review-stop` 으로 끄고 `/auto-review-start` 로 다시 켤 수 있어요(스레드 정리는 계속 동작). 오늘 사용량은 `/help` 하단 표에서 확인하세요.
MD
)
