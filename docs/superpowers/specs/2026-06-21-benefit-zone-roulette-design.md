...
- **광고 추가 스핀**: 리워드 광고 1편 시청 = **스핀 크레딧 +1**(에너지 직접 지급 아님 — 기존 에너지 리워드와 다른 보상 타입). 어뷰징 방지를 위해 **AdMob SSV로 검증**. 리워드 카드의 `nonce 발급 → 광고(customData=nonce) → 적립 폴링` 패턴을 재사용하되, 적립 판정 기준은 `availableSpins`(또는 `spinCredits`) 증가다.
...
상태 매핑:
- 스핀 가능(무료 또는 크레딧 보유): 버튼 활성 `"돌리기 · 오늘 N회"`
- 무료 소진·크레딧 0·한도 미도달: 버튼 → `"광고 보고 한 번 더"`(광고 시청 플로우)
...
- `RouletteStatus`: `spinsUsedToday`, `dailyLimit`, `freeSpinAvailable`, `availableSpins`(보유 크레딧), `remaining`, `resetAtKst`, `segments`(표시용)
- `RouletteSpinResult`: `prize`, `segmentIndex`, `awardedEnergy`
- **`RouletteRepository`** 인터페이스: `getStatus()`, `spin()`, `requestAdSpinNonce()`, `awaitSpinCredited(baselineAvailable)`
  - `FakeRouletteRepository`(스텁): 로컬 `MutableStateFlow<RouletteStatus>` 보유. `spin()`은 §2 확률로 로컬 가중 랜덤 → 결과 반환·카운트 차감. `awaitSpinCredited`는 즉시 +1 모사. 에너지 실지급 없음.
  - `RemoteRouletteRepository`(후속): 위 BE API 호출. 본 슬라이스 범위 밖(스텁만).
- `RouletteStore`: 상태 보유 + `spin`/광고크레딧 오케스트레이션(리워드 카드 `runRewardFlow`에 상응하는 흐름). `@Throws`로 iOS 노출.
...
- 광고 추가 스핀은 `RewardedAdManager`(기존 single) + 스텁 `awaitSpinCredited`로 동작.
...
| 메서드 | 경로 | 설명 |
|---|---|---|
| GET | `/api/roulette/status` | 사용/한도/무료가능/보유크레딧/세그먼트/resetAtKst |
| POST | `/api/roulette/issue-nonce` | 광고 추가 스핀용 SSV nonce |
| POST | `/api/roulette/spin` | 스핀 1개 소모 → 서버 가중추첨 → 에너지 지급 → 결과 반환 |
| (SSV) | AdMob → BE | 광고 검증 시 스핀 크레딧 +1 (에너지 아님) |

## 6. 테스트
- 공유: `FakeRouletteRepository`/`RouletteStore` 단위 테스트 — 확률 가중 분포(시드 고정 또는 주입형 RNG), 무료/크레딧/한도 상태 전이, `spin` 후 카운트 차감, `awaitSpinCredited` 증가 판정.
