 ...
 *
 * On successful completion a terminal `event: done` is appended (data carries the settled reward
 * payload JSON, e.g. `{"pointDelta":1,"expDelta":1}`) so the client can treat end-of-stream as
 * application data and stop reading. Clients key completion on the `done` event NAME (not the data),
 * which makes the stream resilient to the transport-level `RST_STREAM(INTERNAL_ERROR)` that nginx
 * emits over HTTP/2 right after the upstream SSE response closes: the client has already seen `done`
 * and ignores the reset instead of surfacing it as a "connection dropped" error.
 *
